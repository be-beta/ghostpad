...
/** Cor de verdade de cada opcao. O destaque vem do tema, em tempo de desenho. */
export function paint(color: Color, accent: string): string {
  return color === "accent" ? accent : color === "white" ? "#ffffff" : "#141414";
}

/** Halo de contraste: escuro em volta de cor clara, claro em volta do preto. */
function halo(color: Color): string {
  return color === "black" ? "rgba(255, 255, 255, 0.6)" : "rgba(0, 0, 0, 0.38)";
}

/** Preto ou branco, o que tiver mais contraste com a caixa. */
export function inkOn(background: string): string {
  const m = /^#([0-9a-f]{6})$/i.exec(background) ?? null;
  let r: number, g: number, b: number;
  if (m) {
    const n = parseInt(m[1], 16);
    [r, g, b] = [(n >> 16) & 255, (n >> 8) & 255, n & 255];
  } else {
    [r, g, b] = (background.match(/\d+/g) ?? ["0", "0", "0"]).map(Number);
  }
  // Luminancia relativa aproximada, suficiente para escolher entre dois.
...
function tracar(ctx: CanvasRenderingContext2D, color: Color, accent: string, largura: number, caminho: () => void) {
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  ...
  ctx.beginPath();
  caminho();
  ctx.strokeStyle = paint(color, accent);
  ctx.lineWidth = largura;
  ctx.stroke();
}

function desenharSeta(ctx: CanvasRenderingContext2D, obj: Extract<Obj, { kind: "arrow" }>, accent: string) {
  ...
  tracar(ctx, obj.color, accent, TRACO_SETA, () => {
  ...
  ctx.beginPath();
  triangulo();
  ctx.fillStyle = paint(obj.color, accent);
  ctx.fill();
  ctx.strokeStyle = paint(obj.color, accent);
  ctx.lineWidth = 1.5;
  ctx.stroke();
}

function desenharTexto(ctx: CanvasRenderingContext2D, obj: Extract<Obj, { kind: "text" }>, accent: string) {
...
export function drawObject(ctx: CanvasRenderingContext2D, obj: Obj, accent: string): void {
  switch (obj.kind) {
    case "arrow":
      return desenharSeta(ctx, obj, accent);
    case "rect":
      return tracar(ctx, obj.color, accent, TRACO, () => ctx.roundRect(obj.x, obj.y, obj.w, obj.h, RAIO_CAIXA));
    case "circle":
      return tracar(ctx, obj.color, accent, TRACO, () =>
        ctx.ellipse(obj.x + obj.w / 2, obj.y + obj.h / 2, obj.w / 2, obj.h / 2, 0, 0, Math.PI * 2),
      );
    case "text":
      return desenharTexto(ctx, obj, accent);
  }
}
...
export async function renderPng(
  ...
  for (const obj of objs) drawObject(ctx, obj, accent);
